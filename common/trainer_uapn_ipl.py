...
class RunningAverage(object):
    ...
    def average(self):
        ...
        for key, value in self.val_hist.items():
            ...
            v = [tensor.cpu().detach().numpy() if isinstance(tensor, torch.Tensor) else tensor for tensor
                 in value]
...
class ClassifierTrainer(BaseTrainer):
    def __init__(self, model: nn.Module, train_dataloader: DataLoader,
        ...
        if logger is None:
            self.logger = self.init_logger(output_dir, level=logging.INFO)
        else:
            self.logger = logger
        self.writer = writer
        ...
        if resume_from is not None and len(resume_from) > 0:
            self.model, self.optimizer, self.start_epoch = self.resume_model(self.model, self.optimizer, resume_from,
                                                                             self.logger)
    ...
    def train_once(self, epoch_idx, epoch):
        ...
        for iter_idx, data in enumerate(self.train_dataloader):
            if epoch_idx<=math.ceil(epoch*0.7):
                loss_func = self.loss_func1
            else:
                loss_func=self.loss_func2
            rtn = self.batch_process(self.model, data,epoch_idx, train_mode=True,
                                     optimizer=self.optimizer, loss_func=loss_func, **self.batch_param)
    ...
    def eval_once(self, epoch_idx, epoch):
        ...
        for iter_idx, data in enumerate(self.val_dataloader):
            rtn = self.batch_process(self.model, data,epoch_idx, train_mode=False,
                                     loss_func=loss_func, **self.batch_param)
            assert (isinstance(rtn, dict))
        ...
        for epoch_idx in range(start, epoch):
            ...
            if (epoch_idx % self.eval_every == 0 or epoch_idx == epoch - 1):
                ...
                if self.test_func is not None:
                    test_rtn = self.test_func(self.model, self.val_dataloader, self.logger, self.writer, epoch_idx,
                                              **self.batch_param)
