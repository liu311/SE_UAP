...
def SNR_tensor(pred, label):
    assert pred.shape == label.shape, "the shape of pred and label must be the same"
    if len(pred.shape) > 1:
        sigma_s_square = torch.mean(label * label, dim=1)
        sigma_e_square = torch.mean((pred - label) * (pred - label), dim=1)
        snr = 10 * torch.log10(sigma_s_square / sigma_e_square.clamp_min(1e-9))
        snr = torch.mean(snr)
    else:
        sigma_s_square = torch.mean(label * label)
        sigma_e_square = torch.mean((pred - label) * (pred - label))
        snr = 10 * torch.log10(sigma_s_square / sigma_e_square.clamp_min(1e-9))
    return snr
