...
class Generator1D_distributed(nn.Module):
    def __init__(self, noise_dim=100, output_dim=3200):
        super(Generator1D_distributed, self).__init__()
        self.noise_dim, self.output_dim = noise_dim, output_dim
        self.base_channel, self.base_size = 64, 25
        self.transform_layer = nn.Linear(noise_dim, self.base_channel * self.base_size, bias=True)
        self.blocks = self.get_blocks(self.base_channel, self.base_size, output_dim)
        self.out_layer_mu = nn.Conv1d(self.base_channel, 1, 3, 1, 1, bias=False)
        self.out_layer_rho = nn.Conv1d(self.base_channel, 1, 3, 1, 1, bias=False)
        self.last_zero_init()

    def last_zero_init(self):
        if self.out_layer_mu.weight is not None:
            nn.init.constant_(self.out_layer_mu.weight, val=0)
        if self.out_layer_mu.bias is not None:
            nn.init.constant_(self.out_layer_mu.bias, val=0)
        if self.out_layer_rho.weight is not None:
            nn.init.constant_(self.out_layer_rho.weight, val=0)
        if self.out_layer_rho.bias is not None:
            nn.init.constant_(self.out_layer_rho.bias, val=0)
        # self.blocks[-1].zero_init()
    ...
    def reparameterization(self, mu, rho, K=1):
        device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
        # device = torch.device("cpu")

        '''
        :param mu: 分布的均值
        :param rho: 用于计算分布的标准差
        :param K: 采样次数
        :return: 从噪声分布中采样的噪声
        '''
        batch_size = mu.shape[0]
        noise_dim = mu.shape[1]
        if K > 1:
            epsilon = torch.randn(size=(batch_size, noise_dim)).to(device)
            sigma = torch.log1p(torch.exp(rho))
            z = mu + epsilon * sigma
            for _ in range(K - 1):
                epsilon = torch.randn(size=(batch_size, noise_dim))
                sigma = torch.log1p(torch.exp(rho))
                z += mu + epsilon * sigma
        else:
            # 生成均值和方差较小的数
            ...
            # 使用 torch.randn 生成随机数，并进行缩放和平移操作

            # epsilon = torch.randn(size=(batch_size, noise_dim)).to(device)
            epsilon = torch.randn(size=(batch_size, noise_dim)).to(device) * std_dev + mean
            sigma = torch.log1p(torch.exp(rho))
            z = mu + epsilon * sigma
        return z / K

    # def reparameterization2(self,mu, logvar,K=1):
    #     device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
    #     batch_size = mu.shape[0]
    #     noise_dim = mu.shape[1]
    #     epsilon = torch.randn(size=(batch_size, noise_dim)).to(device)
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        x = self.blocks(x)
        x_mu = self.out_layer_mu(x)  # 128,1,3200
        x_mu = x_mu.view(B, -1)  # 128,3200
        # print(x_mu)
        # print(x_mu.shape)
        x_rho = self.out_layer_rho(x)  # 128,1,3200
        x_rho = x_rho.view(B, -1)  # 128,3200

        # print(x_rho)
        # print(x_rho.shape)
        # print(x.shape)
        # x = self.blocks(x)
        x = self.reparameterization(x_mu, x_rho)
        return x.view(B, -1), x_mu, x_rho


class KL_loss(object):
    """
    KL divergence between q(delta|x) = N(mu, sigma^2) and p(delta) = N(0, I)
    """

    def __call__(self, mu, rho):
        sigma = torch.log1p(torch.exp(rho))
        return torch.mean(torch.sum(
            0.5 * (-torch.log(torch.square(sigma) + 1e-7) - 1
                   + torch.square(sigma) + torch.square(mu)), -1))