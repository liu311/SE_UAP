...
def sentence_test_logits(model, wav_data: torch.Tensor, wlen: int = 3200,
    ...
    if length <= wlen:
        padded = torch.zeros(wlen, device=wav_data.device, dtype=wav_data.dtype)
        padded[:length] = wav_data
        pred_all.append(model(padded.unsqueeze(0)))
    else:
        begin = 0
        while begin <= length - wlen:
...
def build_frame_delta(raw_delta: torch.Tensor, args):
    """Project a 0.2 s perturbation for legacy frame-level training."""
    return project_l2(raw_delta, args.tau)
...
def train_one_epoch(generator, speaker_model, detector, loader, optimizer, args, device, epoch):
    ...
    for clean, speaker_id, _ in pbar:
        ...
        for _ in range(args.num_z):
            ...
            delta = build_frame_delta(generator(z), args).expand_as(clean)
...
def validate(generator, speaker_model, detector, file_list: List[str], data_folder: str,
    ...
    for fname in pbar:
        ...
        delta_full = choose_uap_delta(uap_pool, len(audio), args)
...
def main():
    ...
    generator = Generator1D(args.noise_dim, args.frame_dim).to(device)
    ...
    if args.mode == "train":
        ...
        best_ckpt_path = os.path.join(args.output_dir, f"se_uap_generator_{args.dataset}_best.pth")
        for epoch in range(args.epochs):
            ...
            if args.train_objective == "utterance" and val_files:
                ...
                if val_metrics.get("val_score", -float("inf")) > best_score:
        ...
        ckpt_path = os.path.join(args.output_dir, f"se_uap_generator_{args.dataset}_final.pth")
        save_checkpoint(
            generator,
            ckpt_path,
            optimizer=optimizer,
