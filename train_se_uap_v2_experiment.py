#!/usr/bin/env python3
"""
Train and evaluate SE-UAP v2 (P0/P1 improvements over the manuscript baseline).
...
def select_uap_pool(generator, speaker_model, detector, file_list, data_folder,
    ...
    best --pool_size by (FR + val_frd_weight * FRD). Purely offline: the deployed
    attack still draws randomly from the selected pool, so universality is preserved."""
    generator.eval()
    ...
    for ci, raw_delta in enumerate(candidates):
        ...
        n = max(len(audios), 1)
        score = fr_hits / n + args.val_frd_weight * (frd_hits / n)
        scores.append(score)
        if (ci + 1) % 10 == 0:
...
def write_eval_results(metrics: Dict, args, detector_meta: Dict):
    ...
    csv_path = os.path.join(args.output_dir, f"se_uap_v2_eval_{args.dataset}_{time.strftime('%Y%m%d_%H%M%S')}.csv")
    columns = [
        "dataset",
        "detector_model",
        "detector_type",
...
# --------------------------------------------------------------------------- #
def parse_args():
    parser = argparse.ArgumentParser("SE-UAP v2 main experiment (P0/P1 improvements)")
...
def main():
    args = parse_args()
    if args.frame_loss_weight < 0:
        raise ValueError("--frame_loss_weight must be non-negative")
    seed_all(args.seed)
    ...
    if args.mode == "train":
        ...
        best_ckpt_path = os.path.join(args.output_dir, f"se_uap_v2_generator_{args.dataset}_best.pth")
        ...
        ckpt_path = os.path.join(args.output_dir, f"se_uap_v2_generator_{args.dataset}_final.pth")
        save_checkpoint(
            generator,
            ckpt_path,
            optimizer=optimizer,
    ...
    print(
        f"SE-UAP v2 {args.dataset}: FR={eval_metrics['FR']:.4f}, FRD={eval_metrics['FRD']:.4f}, "
